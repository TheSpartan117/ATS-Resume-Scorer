#!/usr/bin/env python3
"""Get detailed parameter scores for all 3 CVs."""
import sys
...
for cv_info in TEST_CVS:
    try:
        ...
        print(f"  ✓ {cv_info['name']}: {result['overallScore']:.1f}/100")
...
def get_param_score(result, param_code):
    ...
    # Check weaknesses and strengths in feedback
    if 'feedback' in result:
        # Check weaknesses
        for weak in result['feedback'].get('weaknesses', []):
            if weak.get('code') == param_code:
                return weak.get('score', 0)

        # Check strengths
        for strength in result['feedback'].get('strengths', []):
            if strength.get('code') == param_code:
                return strength.get('score', 0)
...
for cv_info in TEST_CVS:
    result = results.get(cv_info['name'])
    if result:
        total = result.get('overallScore', 0)
